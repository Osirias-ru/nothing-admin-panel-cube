...
const { searchUser, setVipStatus } = require("../../database");

const scene = new Scenes.BaseScene("setSub");
...
scene.hears(/.*/, async (ctx) => {
  if (ctx.scene.state.nextStep === "awaitingName") {
    ...
    await changeSub(ctx, days);
  }
});

async function changeSub(ctx, days) {
  const newSubDays = await setVipStatus(ctx.session.user.tg_id, days);
  if (newSubDays === null) {
